...
namespace PRC
{
   struct FlatStruct
   {
      enum FlatType {LocalMin=0,LocalMax=1};
      int start;
      int stop;
      FlatType type;
      FlatStruct();
      FlatStruct(int begin, int end, FlatType t);
      void Print(std::ostream &out) const;
      std::string Print() const;
   };

   typedef std::vector<FlatStruct>  FlatStructVec;
}

#include<prc/internal/internal_FlatStruct.h>
