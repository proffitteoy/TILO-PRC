...
namespace PRC
{
   template<typename T, typename S> double gaussSimMatrix(const T &data, S &result, double sigma, GAUSSSIM_ADJ_MODE mode,double eps=1e-8, bool zeroDiagonal=true);
   template<typename T> double findAverageKNNDist(const T &data,  int k);
}
